...
public class Player extends Creature {
	...
	public Player(Handler handler, float x, float y) {
		...
		bounds.height = 35;
		moving = false;
		shot=new Shot(handler, -100, -100, xMove, yMove, this.Id);
		firing = false;
		// Animatons
	...
	private void getInput() {
		...
		if (handler.getKeyManager().right)
			xMove = speed;
		firing=handler.getKeyManager().fire;
		if (firing) {
	...
	private void fire() {
		...
		if (shotCounter == 0) {
			...
			initShot();
			shot = new Shot(handler, shotX, shotY, xMove, yMove, Id);
			handler.getWorld().getEntityManager().addEntity(shot);
